var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    onAfterRender: function (target) {
        ...
        var vcBody = vc.find('div.datagrid-body');
        vcBody.children('div.datagrid-empty').remove();
        if (!$(target).datagrid('getRows').length) {
            var d = $('<div class="datagrid-empty"></div>').html(opts.emptyMsg || '没有符合筛选条件的数据').appendTo(vcBody[1]);
            d.css({ position: 'absolute', left: 0,  textAlign: 'center', fontSize: '14px' });
            //vc.css({ height: 80 });
            //vc.find('div.datagrid-body').css({ height: 100 });
        }
    }
});
